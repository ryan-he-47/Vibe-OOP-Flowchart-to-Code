...
class NodeItem(QGraphicsRectItem):
    ...
    def refresh_texts(self) -> None:
        ...
        if self.inputs:
            chunks.append(f"IN: {', '.join(self.inputs)}")
        if self.outputs:
            chunks.append(f"OUT: {', '.join(self.outputs)}")
        if self.members:
            chunks.append(f"MEM: {', '.join(self.members)}")
    ...
    def _paint_ports(self, painter) -> None:
        ...
        for idx, name in enumerate(self.outputs):
            ...
            painter.drawEllipse(QPointF(x, y), radius, radius)
            painter.drawText(QPointF(x - 130, y + 4), name[:20])
    ...
    def mouseReleaseEvent(self, event) -> None:
        ...
        if candidate_parent is not None and candidate_parent is not current_parent:
            ...
            if not current_parent.sceneBoundingRect().contains(self.scene_center()):
